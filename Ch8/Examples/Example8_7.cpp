...
// at each 10 units interval of time.
...
void print(const double list[], int length, const double avgSpeed[]);
...
void print(const double list[], int length, const double avgSpeed[])
{
    ...
    for (int index = 1; index < length; index++)
        cout << setw(5) << index *  10 << setw(14) << list[index] << setw(6) << " " << setw(10)
            << avgSpeed[index - 1] << "  [" << (index - 1) * 10 << ", " << index * 10 << "]" << endl;
}